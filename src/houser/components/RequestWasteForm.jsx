// src/houser/components/RequestWasteForm.jsx
...
export default function RequestWasteForm({ onSuccess }) {
  const { token, user } = useContext(AuthContext);
  const fileInputRef = useRef(null);

  const [materials, setMaterials] = useState([
    { wasteType: 'Plastic', quantity: '', unit: 'kg' }
  ]);
  ...
  const addMaterialRow = () => {
    setMaterials([...materials, { wasteType: 'Plastic', quantity: '', unit: 'kg' }]);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        'https://waste-management-3-iw0g.onrender.com/api/waste',
        {
      ...
      setSuccessMsg('Waste collection request created successfully!');
      setMaterials([{ wasteType: 'Plastic', quantity: '', unit: 'kg' }]);
      setAddress('');
      setCollectionNote('');
      setImageBase64(null);
      if (onSuccess) onSuccess(response.data);
    } catch (err) {
      setErrorMsg(err?.response?.data?.message || err?.response?.data?.error || 'Failed to create request.');
    } finally {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 max-w-2xl mx-auto my-6">
      <div className="flex items-center gap-3 mb-6 pb-4 border-b border-gray-100">
        ...
        <div>
          <h2 className="text-xl font-bold text-gray-900">Request Waste Collection</h2>
          <p className="text-sm text-gray-500">Fill out details for household waste pickup</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
            ...
            {materials.map((mat, idx) => (
              <div key={idx} className="flex flex-col sm:flex-row items-center gap-3 p-3 bg-gray-50 rounded-xl">
                <div className="w-full sm:w-1/2">
                  <select
                    value={mat.wasteType}
                    onChange={(e) => handleMaterialChange(idx, 'wasteType', e.target.value)}
          ...
          <button
            ...
            className="mt-3 text-sm text-emerald-700 font-medium flex items-center gap-1.5 hover:underline cursor-pointer"
          >
        ...
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-1">
            Instructions / Collection Note (Optional)
          </label>
          <textarea
            rows="2"
            placeholder="Provide specific directions or pickup notes for the collector..."
        ...
        <div>
          ...
          {imageBase64 ? (
            <div className="relative w-32 h-32 rounded-xl overflow-hidden border border-gray-200">
              <img src={imageBase64} alt="Waste preview" className="w-full h-full object-cover" />
              <button
                type="button"
            ...
            >
              <ImagePlus className="h-6 w-6 mb-1 text-emerald-600" />
              <span className="text-xs font-medium">Click to upload photo of waste</span>
            </button>
          )}
          <input
            type="file"
        ...
        >
          {isSubmitting ? (
            ...
            <>
              <Send className="h-4 w-4" /> Submit Waste Request
            </>
          )}
        </button>
      </form>
